...
data <- original_data %>% select(-c(X,date,covid,Latitude.x,Longitude.x,covid,county_name,state_code,WIN2016))

data_county <- data %>% group_by(FIPS) %>% summarise(county_population_density=max(county_population)/max(Square.Miles),          daily_distance_diff=min(daily_distance_diff),                                                     encounters_rate=min(encounters_rate),                                                                                 avg_hh_size=max(Average.Household.Size),                                                     living_alone=max(X..Living.Alone),                                                     bach_degree=max(X..Bachelors.Degree),                                                     grad_degree=max(X..Graduate.Degree),                                                     internet_access=max(X..Broadband),                                                     mhhi=max(Median.Household.Income),                                                     mean_min_to_work=max(Mean.Minutes.To.Work),gini=max(Gini.Index),                                                     unemployment=max(X..Unemployment.Rate),median_age=max(Median.Age),                                                     perc_dem_2016=max(PCDEM2016),perc_gop_2016=max(PCGOP2016),                                                     perc_obesity=max(PCOBESITY),                                                     establishments=max(All.Establishments),                                                     employees=max(All.Employees),payroll=max(All.Payroll)) 
...
data_county <- inner_join(data_county,religion_data_small,by = 'FIPS')
...
data_county <- data_county %>% mutate(party=case_when((data_county$perc_dem_2016-data_county$perc_gop_2016) < 0 ~ "gop",
...
features <- data_county %>% select(-c(FIPS,party))
